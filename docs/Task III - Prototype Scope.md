...
The RoomLight prototype is a Python command‑line tool that simulates a set of rooms and their lighting states. It lets a user create one lighting configuration and apply it to all rooms or selected rooms at once, demonstrating “configure once, sync everywhere.” The tool also allows viewing the lighting state of every room with a single command, proving that room verification can be done quickly.
